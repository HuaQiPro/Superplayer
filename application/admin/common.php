...
if (!function_exists('ec_arr2file')) {
    function ec_arr2file($f,$arr='')
    {
        if(is_array($arr)){
...
if (!function_exists('ec_write_file')) {
    function ec_write_file($f,$c='')
    {
        $dir = dirname($f);
        if(!is_dir($dir)){
            ec_mkdirss($dir);
...
if (!function_exists('ec_mkdirss')) {
    function ec_mkdirss($path,$mode=0777)
    {
        if (!is_dir(dirname($path))){
            ec_mkdirss(dirname($path));
        }
        if(!file_exists($path)){
