let PlayEr = {
    ...
    "play": function(){
        ...
        if(dp["chromecast"] === '1'){
            playData["chromecast"] = true;
        }
        let u=PlayEr.ad.uic(ConFig['url']),t = PlayEr.type(u);
        playData["theme"] = ConFig['config']['gx']['color'];
        playData["lang"] = dp["lang"];
        playData["preload"] = dp["preload"];
        playData["volume"] = dp["volume"];
        playData["playbackSpeed"] = dp["playbackSpeed"].split(',');
        playData["video"]["url"] = u;
        playData["video"]["type"] = t;
        if(t === 'flv') {
            $("title").after('<script src="../player/js/flv.min.js"></script>');
        }else if(t === 'm3u8' || t === 'hls'){
            $("title").after('<script src="../player/js/hls.min.js"></script>');
        }
        let contextmenu = dp["contextmenu"].split('#');
        for (let i=0; i<contextmenu.length; i++) {
            let t = contextmenu[i].split('$');
            playData['contextmenu'][i] = {
                "text": t[0],
                "link": t[1]
            }
        }

        PlayEr.void = new DPlayer(playData)
    },
    "Init":function(){
        ...
        if(html.length > 10){
            ...
            $(".dplayer-setting-origin-panel").append('<div class="dplayer-setting-item get-nav"><span class="dplayer-label">'+lg['nav']+'</span></div>');
            $(".get-nav").click(function(){
                box.toggleClass('nav-off');
            });
        }
        console.log(
            "\n" +
    ...
    "load" : function(){
        ...
        $(".dplayer-video-wrap").append('<div class="ec-danMa text"><div class="ec-danMa-item ec-danMa-item--demo"></div></div><div class="ec-subtitle"></div>');
        box.prepend('<div class="lock-box"></div><div class="header ease flex between"><div class="player-title"></div><div class="flex qoe-normal" style="display:none"><div class="kui-time"></div><div class="batteryShape"><div class="level"><div class="percentage"></div></div></div></div></div>' +
            ...
            '<button class="dm-send" data-balloon="'+lg['dm-send']+'" data-balloon-pos="up">'+lg['dm-send']+'</button></div></div><div class="player-list-off off"></div><div class="ec-box player-list"><div class="new-check"><div class="new-body"></div></div></div><div class="ec-remember"></div><div class="broadside seat'+ConFig['config']['seat']+'"></div>');
        $(".dplayer-icons-right").prepend('<div class="dplayer-icon list-bnt"><span class="dplayer-icon-content">'+lg['list']+'</span></div>' +
            '<div class="dplayer-icon dm-bnt"><span class="dplayer-icon-content">'+PlayEr.svg.dm+'</span></div>' +
            '<div class="dplayer-icon content-bnt"><span class="dplayer-icon-content">'+PlayEr.svg.content+'</span></div>');
        ...
        if(ConFig['config']['autoOrientation'] === '1'){
            PlayEr.void.on('fullscreen', function (a) {
                PlayEr.autoOrientation(true);
            });
            PlayEr.void.on('fullscreen_cancel', function (a) {
    ...
    "header":{
        ...
        "time":function(){
            ...
            PlayEr.void.on('fullscreen', function () {
                $(".header .qoe-normal").show();
            });
            PlayEr.void.on('fullscreen_cancel', function () {
    ...
    "danMu": {
        ...
        "Init": function () {
            ...
            switch (ConFig['config']['danMu_input']) {
                ...
                case "2":
                    dm_box.hide();
                    PlayEr.void.on('fullscreen', function () {dm_box.show();});
                    PlayEr.void.on('fullscreen_cancel', function () {dm_box.hide();});
                    break;
            }
        ...
        "_danAnimation": function (position) {
            ...
            const isFullScreen = !!PlayEr.void.fullScreen.isFullScreen();
            const animations = {
            ...
            PlayEr.void.on('loadedmetadata', function () {
                ...
                setTimeout(function() {
                    ...
                    let time = setTimeout(function() {
                        $("#loading").hide();
                        if(ConFig['config']['dp']["autoplay"] === '1'){
                            PlayEr.void.play();
                        }
                    }, (wait*1000));
